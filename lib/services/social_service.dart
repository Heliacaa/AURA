...
class SocialService {
  ...
  Future<void> incrementChallengeProgress(String type, int amountToAdd) async {
    try {
      ...
      debugPrint('⏳ incrementChallengeProgress tetiklendi. type: $type, amount: $amountToAdd, uid: $uid');
      ...
      debugPrint('Bulunan challenge sayısı (katılımcı olduğun): ${query.docs.length}');
      ...
      for (var doc in query.docs) {
        ...
        if (docType == null || docType == 'unknown') {
          if (type == 'water' && (unit.contains('bardak') || unit.contains('su') || title.toLowerCase().contains('su'))) {
            isMatch = true;
          } else if (type == 'steps' && (unit.contains('adım') || unit.contains('adim') || title.toLowerCase().contains('adım'))) {
            isMatch = true;
          }
        }
        ...
        if (isMatch) {
          ...
          debugPrint('⏭️ Eşleşmedi: $title. Beklenen: $type, Gelen type: $docType, Unit: $unit');
        }
      }

      if (matchCount == 0) {
        debugPrint('⚠️ Bu işlem tipinde ($type) eşleşen ve katılımcısı olduğunuz challenge bulunamadı. Lütfen önce Topluluk sekmesinden meydan okumaya KATIL (Ayrıl yazıyorsa katılmışsındır) tuşuna basıp basmadığını kontrol et.');
      }
    } catch (e) {
  ...
  Future<void> generateMockData() async {
    ...
    if (snap.docs.isEmpty) {
      await challengesRef.add({
        ...
        'description': "Topluluk olarak bu hafta toplam 1 milyon adım atıyoruz. Sen de katıl!",
        'type': "steps",
      ...
      await challengesRef.add({
        'title': "Susuz Kalma!",
        'description': "Bugün toplam 5.000 bardak su içme hedefine ulaşırsak sürpriz sandık açılacak.",
        'type': "water",
        'currentAmount': 1500,
        'targetAmount': 5000,
        'unit': "Bardak",
    ...
    final actSnap = await activitiesRef.limit(1).get();
    if (actSnap.docs.isEmpty) {
      await activitiesRef.add({
        'userId': 'mock_user_1',
        'userName': 'Ahmet',
        'userAvatarUrl': '',
        'actionTitle': '🔥 10 Günlük Seri Başarısı!',
        'actionDescription': 'Ahmet uygulamaya 10 gün arka arkaya girerek ateş rozetini geliştirdi.',
